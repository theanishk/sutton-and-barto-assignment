"""
Value Function Approximation in a 1000-State Random Walk

Features:
- True value computation via dynamic programming
- Gradient Monte Carlo learning
- Semi-gradient n-step Temporal Difference (TD) learning
- Aggregation, tile coding, polynomial, and Fourier basis function approximators
- RMSE analysis and visualization for each approach

Figures:
- 9.1: Gradient Monte Carlo with state aggregation
- 9.2: Semi-gradient TD with different step sizes and alphas
- 9.5: Monte Carlo with polynomial and Fourier bases
- 9.10: Monte Carlo with tile coding vs. state aggregation
...
# --- Value Function Estimators and Utilities ---


def compute_true_value():
    """
    Computes the true state values using dynamic programming.

    Returns
    -------
    true_value : np.ndarray
        Array of true state values from state 0 to 1001.
    """
...
def step(state, action):
    """
    Simulates an environment step based on current state and action.
    ...
    state : int
        Current state.
    action : int
        Action taken (-1 for left, +1 for right).
...
def figure_9_5(true_value):
    ...
    for run in range(runs):
        for i in range(len(orders)):
            value_functions = [
                BasesValueFunction(orders[i], POLYNOMIAL_BASES),
            ...
            for j in range(len(value_functions)):
                for episode in tqdm(range(episodes)):
                    ...
                    errors[j, i, episode] += np.sqrt(
                        np.mean(np.power(true_value[1:-1] - state_values, 2))
                    )
...
def figure_9_10(true_value):
    ...
    for run in range(runs):
        ...
        value_functions = [
            TilingsValueFunction(num_of_tilings, tile_width, tiling_offset),
