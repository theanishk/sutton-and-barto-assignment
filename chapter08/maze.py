"""
Dyna-Q and Prioritized Sweeping Maze Planning Algorithms

This module contains implementations for reinforcement learning algorithms,
including Dyna-Q, Dyna-Q+, and Prioritized Sweeping. These algorithms are applied
to grid-world mazes to simulate learning and planning in dynamic environments.

Key Components:
- `Maze`: Represents the grid-based environment.
- `DynaParams`: Encapsulates hyperparameters for Dyna-based algorithms.
- `TrivialModel`, `TimeModel`, `PriorityModel`: Models used for planning.
- `dyna_q`, `prioritized_sweeping`: Core implementations of the respective algorithms.
- `figure_8_2`, `figure_8_4`, `figure_8_5`: Experimental setups visualizing agent performance.
"""
...
class Maze:
    ...
    def extend_maze(self, factor):
        ...
        new_maze.START_STATE = [
            self.START_STATE[0] * factor,
            self.START_STATE[1] * factor,
        ]
        ...
        new_maze.q_size = (
            new_maze.WORLD_HEIGHT,
            new_maze.WORLD_WIDTH,
            len(new_maze.actions),
        )
...
def dyna_q(q_value, model, maze, dyna_params):
    ...
    while state not in maze.GOAL_STATES:
        ...
        q_value[state[0], state[1], action] += dyna_params.alpha * (
            reward
            + dyna_params.gamma * np.max(q_value[next_state[0], next_state[1], :])
        ...
        for t in range(0, dyna_params.planning_steps):
            ...
            q_value[state_[0], state_[1], action_] += dyna_params.alpha * (
                reward_
                + dyna_params.gamma * np.max(q_value[next_state_[0], next_state_[1], :])
    ...
    while state not in maze.GOAL_STATES:
        ...
        while planning_step < dyna_params.planning_steps and not model.empty():
            ...
            for state_pre, action_pre, reward_pre in model.predecessor(state_):
                priority = np.abs(
                    reward_pre
                    + dyna_params.gamma * np.max(q_value[state_[0], state_[1], :])
    ...
    for run in tqdm(range(dyna_params.runs)):
        ...
        for i in range(len(dyna_params.methods)):
            ...
            while steps < max_steps:
                ...
                rewards[run, i, min(steps, max_steps - 1)] = (
                    rewards[run, i, last_steps] + 1
                )
                last_steps = steps
    ...
    for run in range(0, runs):
        for i in range(0, len(method_names)):
            for mazeIndex, maze in zip(range(0, len(mazes)), mazes):
                print(
                    "run %d, %s, maze size %d"
                    % (run, method_names[i], maze.WORLD_HEIGHT * maze.WORLD_WIDTH)
                )
